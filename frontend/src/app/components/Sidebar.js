    ...
    <div className="w-64 bg-white border-r border-gray-200 flex flex-col">
      ...
      <nav className="flex-1 p-4 space-y-2">
        {menuItems.map((item) => (
          <button
            key={item.id}
            className={`w-full flex items-center gap-2 px-4 py-2 rounded ${activeTab === item.id ? "bg-green-600 text-white" : "hover:bg-gray-100"}`}
      ...
      <div className="p-4 border-t border-gray-200">
        <Link href="/">
          <button className="w-full flex items-center justify-center border rounded px-4 py-2 gap-2 hover:bg-gray-100">
